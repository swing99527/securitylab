...
console.log("🔍 API Configuration:", {
  ...
  ENV_API_BASE: process.env.NEXT_PUBLIC_API_BASE_URL,
})
// 模拟延迟
...
export const projectApi = {
  ...
  async create(data: {
    ...
    if (USE_MOCK) {
      ...
      const newProject: Project = {
        id: `PRJ-${Date.now()}`,
        code: `PRJ-${new Date().getFullYear()}-${String(Math.floor(Math.random() * 1000)).padStart(3, '0')}`,
        name: data.name,
        client: data.client,
        ...
        assignee: data.engineerId || '',
      }
      return { code: 200, message: 'success', data: newProject }
    }

    try {
      // Get current user ID from localStorage for manager_id
      const userStr = localStorage.getItem('auth_user')
      let managerId = data.engineerId || null
      if (!managerId && userStr) {
        try {
  ...
  async update(
    ...
    if (USE_MOCK) {
      ...
      Object.assign(project, {
        ...data,
        endDate: data.deadline || project.endDate,
      })
  ...
  // 重新计算项目进度
  async recalculateProgress(id: string): Promise<ApiResponse<{ progress: number; message: string }>> {
    if (USE_MOCK) {
      await delay(500)
      const project = mockProjects.find((p) => p.id === id)
      if (!project) {
        return { code: 404, message: '项目不存在', data: null as any }
      }

      // Mock: simulate calculation based on random completion
      const mockProgress = Math.floor(Math.random() * 100)
      project.progress = mockProgress
    ...
    try {
      const response = await request(`/api/v1/projects/${id}/recalculate-progress`, {
        method: 'POST',
      })
      return {
        code: 200,
...
export const taskApi = {
  ...
  async getDetail(id: string): Promise<ApiResponse<TaskDetail>> {
    if (USE_MOCK) {
      ...
      if (!task) {
        return { code: 404, message: "任务不存在", data: null as unknown as TaskDetail }
      }
      return {
        code: 200,
        message: "success",
...
export const reportApi = {
  ...
  // 创建报告
  async create(data: any): Promise<ApiResponse<Report>> {
    ...
    const response = await request("/api/v1/reports", {
      method: "POST",
  ...
  async getDetail(id: string): Promise<ApiResponse<Report>> {
    if (USE_MOCK) {
      ...
      if (!report) {
        return { code: 404, message: "报告不存在", data: null as unknown as Report }
      }
    ...
    // 转换response的键从snake_case到camelCase
    const transformedData = {
      ...
      updatedAt: response.updated_at,
      approvedAt: response.approved_at,
      // content保持原样,因为它是JSONB，sections已经在里面了
      content: response.content
    }
  ...
  async sign(id: string, data: ReportSignRequest): Promise<ApiResponse<Report>> {
    ...
    if (data.password !== "123456") {
      return { code: 401, message: "密码错误", data: null as unknown as Report }
    }
    const report = mockReports.find((r) => r.id === id)
    if (report) report.status = "signed"
    return { code: 200, message: "签署成功", data: report! }
  },
...
export const sampleApi = {
  ...
  async getDetail(id: string): Promise<ApiResponse<SampleDetail>> {
    if (USE_MOCK) {
      ...
      if (!sample) {
        return { code: 404, message: "样品不存在", data: null as unknown as SampleDetail }
      }
  ...
  async scanQuery(code: string): Promise<ApiResponse<Sample>> {
    ...
    if (!sample) {
      return { code: 404, message: "样品不存在", data: null as unknown as Sample }
    }
    return { code: 200, message: "success", data: sample }
  },
...
export const dashboardApi = {
  async getStats(): Promise<ApiResponse<DashboardStats>> {
    ...
    try {
      const response = await request<DashboardStats>("/api/v1/dashboard/stats")
      return { code: 200, message: "success", data: response }
    } catch (error) {
      return handleApiError(error)
    }
  ...
  async getHeatmap(): Promise<ApiResponse<HeatmapData[]>> {
    ...
    try {
      const response = await request<HeatmapData[]>("/api/v1/dashboard/heatmap")
      return { code: 200, message: "success", data: response }
    } catch (error) {
      return handleApiError(error)
    }
  ...
  async getVulnerabilityTrend(): Promise<ApiResponse<VulnerabilityTrendData[]>> {
    ...
    try {
      const response = await request<VulnerabilityTrendData[]>("/api/v1/dashboard/vulnerability-trend")
      return { code: 200, message: "success", data: response }
    } catch (error) {
      return handleApiError(error)
    }
  ...
  async getTodos(): Promise<ApiResponse<TodoItem[]>> {
    ...
    try {
      const response = await request<TodoItem[]>("/api/v1/dashboard/todos")
      return { code: 200, message: "success", data: response }
    } catch (error) {
      return handleApiError(error)
    }
...
export const knowledgeApi = {
  ...
  async getArticleDetail(id: string): Promise<ApiResponse<Article>> {
    ...
    if (!article) {
      return { code: 404, message: "文章不存在", data: null as unknown as Article }
    }
    // 增加阅读量
    article.views++
    return {
